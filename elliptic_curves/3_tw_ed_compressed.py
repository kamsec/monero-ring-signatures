...
# Third implementation of the elliptic curves arithmetic
...
p = 13; a = -1; d = 2
b = 8  # for compression / decompression - for b=8, x is in (1, 2**7) - upper limit has to be equal / greater than p
...
class Point:
    ...
    def decompress(compressed_point, EC): # compressed_point = y with msb set to x's lsb
        ...
        y_msb = int(bin(compressed_point)[2:].zfill(b)[0])  # if msb was 0 then zfill needed
        y = int('0' + bin(compressed_point)[2:].zfill(b)[1:], 2)
        u = (y**2-1) % p
        v = ((d*y**2)+1) % p
        z = ((u*v**3)*pow((u*v**7), ((p-5)*inv(8))%p, p)) % p
        vz2 = (v*z**2) % p
        if vz2 == u:
            x = z
        elif vz2 == -u % p:
            x = (z*pow(2, ((p-1)*inv(4))%p, p)) % p
        else:
...
def test_compression():
    ...
    G = EC.point(2, 4)
    some_points = [x*G for x in range(20)]
